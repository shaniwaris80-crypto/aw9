...
export function buildInvoicePdf(inv,client,{blob=false}={}){
  ...
  doc.setFont('helvetica','bold');doc.setFontSize(credit?17:21);doc.text(title,14,18);doc.setFontSize(10);doc.text(upper(inv.number||'BORRADOR'),196,16,{align:'right'});doc.setFont('helvetica','normal');doc.text(`FECHA: ${fmtDate(inv.date)}`,196,22,{align:'right'});
  if(credit&&inv.originalInvoiceNumber){doc.setFontSize(8);doc.text(`RECTIFICA: ${upper(inv.originalInvoiceNumber)}`,14,24)}
  ...
  doc.setDrawColor(60);doc.line(140,y,196,y);y+=6;doc.setFont('helvetica','bold');doc.setFontSize(13);doc.text(credit?'TOTAL RECTIFICACIÓN':'TOTAL FACTURA',140,y);doc.text(money(inv.total),196,y,{align:'right'});y+=7;doc.setFontSize(9);doc.setFont('helvetica','normal');
  if(!credit){doc.text(`PAGADO: ${money(inv.paid)}`,140,y);doc.text(`PENDIENTE: ${money(inv.pending)}`,196,y,{align:'right'});y+=6;doc.text(`FORMA DE PAGO: ${upper(inv.paymentMethod||client?.paymentMethod||'PENDIENTE')}`,140,y)}
