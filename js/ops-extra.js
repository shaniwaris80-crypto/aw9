import {Store,st,client,refresh,kpi,table,esc,n,money,upper,isoDate,fmtDate,toast,modal,closeModal,round2,calcInvoice,invoiceStatus,clientBalance} from './context.js';
...
export function client360Modal(c){
  if(!c)return;
  const invoices=st().invoices.filter(i=>i.clientId===c.id).slice().sort((a,b)=>String(b.date||'').localeCompare(String(a.date||'')));
  ...
  const m=modal(`CLIENTE 360º · ${upper(c.name)}`,`<div class="kpis">${kpi('FACTURADO',money(billed))}${kpi('IVA',money(iva))}${kpi('COBRADO',money(paid))}${kpi('PENDIENTE',money(debt),'','warn')}${kpi('FACTURAS',String(invoices.length))}${kpi('PEDIDOS',String(orders.length))}</div><div class="grid2"><section class="panel"><h3>DATOS</h3><p><b>${esc(c.name)}</b></p><p>NIF/CIF: ${esc(c.nif||'—')}</p><p>${esc(c.address||'')}</p><p>${esc(c.phone||'')}</p><p>${esc(c.email||'')}</p><p>FORMA PAGO: ${upper(c.paymentMethod||'')}</p><p>LÍMITE CRÉDITO: ${money(c.creditLimit||0)}</p><button class="btn primary" id="editClientData">EDITAR DATOS Y PRECIOS</button></section><section class="panel"><h3>PRECIOS ESPECIALES</h3>${Object.entries(c.prices||{}).length?Object.entries(c.prices||{}).map(([pid,x])=>`<p>${esc(st().products.find(p=>p.id===pid)?.name||pid)} · <b>${money(x.price)}</b></p>`).join(''):'<p>SIN PRECIOS ESPECIALES</p>'}</section></div><h3>HISTORIAL DE FACTURAS</h3>${table(['FECHA','Nº','TOTAL','IVA','PAGADO','PENDIENTE','ESTADO'],invoices.slice(0,40).map(i=>{const x=calcInvoice(i);return `<tr><td>${fmtDate(i.date)}</td><td>${esc(i.number||'BORRADOR')}</td><td>${money(x.total)}</td><td>${money(x.vatTotal)}</td><td>${money(x.paid)}</td><td>${money(x.pending)}</td><td>${invoiceStatus(i)}</td></tr>`}))}<h3>ÚLTIMOS PEDIDOS</h3>${table(['FECHA','ESTADO','LÍNEAS'],orders.slice(0,20).map(o=>`<tr><td>${fmtDate(o.date)}</td><td>${upper(o.status||'')}</td><td>${o.lines?.length||0}</td></tr>`))}`,{wide:true});
