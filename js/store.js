...
let state=blank(),user=null,role='none',ready=false,online=false;
const listeners=new Set();
const cacheKey='arw2026_cache_v1';
function emit(){listeners.forEach(fn=>fn(state,{user,role,ready,online}))}
...
async function ensureOwner(){const r=ref('members',user.uid),snap=await getDoc(r);if(user.uid===ARW.ownerUid){const x={id:user.uid,email:user.email||ARW.ownerEmail,name:'ADMINISTRADOR',role:'owner',active:true,updatedAt:nowISO()};await setDoc(r,x,{merge:true});role='owner'}else if(snap.exists()&&snap.data().active){role=snap.data().role||'viewer'}else role='none'}
...
async function load(){
  ...
  await seed();online=true;cache();emit();
}
authApi.onChange(async u=>{user=u;ready=false;online=false;if(!u){role='none';ready=true;emit();return}try{await ensureOwner();if(role==='none')throw new Error('SIN PERMISO');await load()}catch(e){console.error(e);online=false}ready=true;emit()});
export const Store={get state(){return state},get user(){return user},get role(){return role},get ready(){return ready},get online(){return online},subscribe(fn){listeners.add(fn);return()=>listeners.delete(fn)},login:authApi.login,logout:authApi.logout,save,remove,bulkSave,saveSettings,audit,load,uid,isoDate,can(scope){if(['owner','admin','manager'].includes(role))return true;return role===scope}};
