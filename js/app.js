...
import {authApi,ensureMasterData,subscribeCollections,saveEntity,userRole} from './firebase.js';
import {upper,normalize,calcInvoice} from './domain.js';
...
const NAV=[
['dashboard','⌂','INICIO',['owner','admin','manager','billing','warehouse','delivery']],['orders','🛒','PEDIDOS / REPARTO',['owner','admin','manager','billing','warehouse','delivery']],['invoices','🧾','FACTURACIÓN',['owner','admin','manager','billing']],['send','📤','ENVÍO DEL DÍA',['owner','admin','manager','billing']],['payments','💶','COBROS',['owner','admin','manager','billing']],
['clients','👤','CLIENTES 360º',['owner','admin','manager','billing','delivery']],['products','🍌','PRODUCTOS',['owner','admin','manager','billing','warehouse','delivery']],['stock','📦','STOCK',['owner','admin','manager','billing','warehouse']],['purchases','📥','COMPRAS',['owner','admin','manager','billing','warehouse']],['suppliers','🏭','PROVEEDORES',['owner','admin','manager','billing','warehouse']],
['operations','🔄','OPERACIONES STOCK',['owner','admin','manager','warehouse']],['documents','📄','PRESUPUESTOS / ALBARANES',['owner','admin','manager','billing','delivery']],['routes','🚚','RUTAS',['owner','admin','manager','billing','warehouse','delivery']],['prices','📈','PRECIOS MASIVOS',['owner','admin','manager','billing']],['expenses','💸','GASTOS',['owner','admin','manager','billing']],
['reports','📊','REPORTES / IVA',['owner','admin','manager','billing']],['closures','🔒','CIERRES',['owner','admin','manager','billing']],['audit','📜','AUDITORÍA',['owner','admin','manager']],['settings','⚙','CONFIGURACIÓN',['owner','admin','manager','billing','warehouse','delivery']]
];
...
const allowedNav=()=>NAV.filter(x=>x[3].includes(Runtime.role));
function navHtml(){return allowedNav().map(([id,ico,label])=>`<button class="nav-btn ${Runtime.view===id?'active':''}" data-view="${id}"><span>${ico}</span><span class="nav-label">${label}</span></button>`).join('')}
function mobileNav(){const pref=[['dashboard','🏠','INICIO'],['orders','🛒','PEDIDOS'],['invoices','🧾','FACTURAR'],['stock','📦','STOCK'],['settings','•••','MÁS']];const allowed=new Set(allowedNav().map(x=>x[0]));return pref.filter(x=>allowed.has(x[0])).map(([id,ico,l])=>`<button data-view="${id}"><b>${ico}</b>${l}</button>`).join('')}
function ensureAllowedView(){const allowed=allowedNav().map(x=>x[0]);if(!allowed.includes(Runtime.view))Runtime.view=allowed[0]||'settings'}
function render(){
  if(!Runtime.user)return;ensureAllowedView();
  ...
  try{content.innerHTML=(VIEWS[Runtime.view]||settingsView)()}catch(e){console.error(e);content.innerHTML=`<div class="danger">ERROR DE PANTALLA: ${String(e.message||e)}</div>`}
  bindDynamic();
...
function go(v){if(!allowedNav().some(x=>x[0]===v))return toast('TU ROL NO TIENE ACCESO A ESTE MÓDULO','bad');Runtime.view=v;app.classList.remove('side-open');render()}
function stateError(name,e){console.error(name,e);cloud.innerHTML=`<span class="sync-bad">ERROR ${upper(name)} · ${upper(e.code||e.message)}</span>`}
...
async function startUser(user){
  ...
  try{
    Runtime.user=user;login.classList.add('hidden');app.classList.remove('hidden');cloud.textContent='CARGANDO FIRESTORE…';
    await ensureMasterData(user);Runtime.role=await userRole(user);if(Runtime.role==='none')throw new Error('USUARIO SIN ROL ACTIVO EN ARW2026');
    userInfo.innerHTML=`${user.email||''} · <span class="role-chip">${upper(Runtime.role)}</span>`;
    if(unsubscribe){unsubscribe();unsubscribe=null}
    unsubscribe=await subscribeCollections((s)=>{
      const {role,...state}=s;Runtime.state={...Runtime.state,...state};Runtime.role=role||Runtime.role;Runtime.syncReady=true;Runtime.lastSyncAt=new Date().toISOString();
      cloud.innerHTML=`<span class="sync-ok">● FIRESTORE · ${Runtime.state.products.filter(x=>!x.archived).length} PRODUCTOS · ${Runtime.state.clients.filter(x=>!x.archived).length} CLIENTES · ${upper(Runtime.role)}</span>`;
      if(modalOpen())Runtime.pendingRender=true;else render();
    },stateError,user);
    render();
  }catch(e){console.error(e);cloud.innerHTML=`<span class="sync-bad">ERROR: ${upper(e.code||e.message)}</span>`;content.innerHTML=`<div class="danger"><b>NO SE PUDO CONECTAR A FIRESTORE.</b><br>${String(e.code||e.message)}<br><br>REVISA REGLAS, USUARIO Y ROL.</div>`}
  finally{booting=false}
}
function stopUser(){Runtime.user=null;Runtime.role='none';Runtime.syncReady=false;unsubscribe?.();unsubscribe=null;app.classList.add('hidden');login.classList.remove('hidden')}
...
function bindTableSearch(input,key){if(!input)return;const apply=()=>{window[key]=input.value;const q=normalize(input.value);content.querySelectorAll('.table-wrap tbody tr').forEach(tr=>{tr.hidden=Boolean(q)&&!normalize(tr.textContent).includes(q)})};input.oninput=apply;apply()}
...
function bindDynamic(){
  ...
  content.querySelectorAll('[data-inv-pdf]').forEach(b=>b.onclick=()=>{const i=selectedInvoice(b.dataset.invPdf),c=Runtime.client(i.clientId)||i.clientSnapshot;downloadInvoice(i,c,i.issuerSnapshot||Runtime.settings())});
  ...
  const rmode=content.querySelector('#reportMode'),rval=content.querySelector('#reportValue');if(rmode)rmode.onchange=e=>{window.ARW_REPORT_MODE=e.target.value;render()};if(rval)rval.onchange=e=>{window.ARW_REPORT_VALUE=e.target.value;render()};
...
function calcTotal(i){return new Intl.NumberFormat('es-ES',{style:'currency',currency:'EUR'}).format(calcInvoice(i).total)}
